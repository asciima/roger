...
namespace roger {
	...
	void dial_server_by_ip(WWRP<forward_ctx> const& fctx) {
		...
		dial_f->add_listener([fctx](WWRP<wawo::net::channel_future> const& f) {
			...
			fctx->ch_stream_ctx->event_poller()->execute([fctx,rt]() {
				...
				const u64_t dns_lookup_time = fctx->ts_dns_lookup_done - fctx->ts_dns_lookup_start;
				WAWO_ASSERT(fctx->ts_server_connect_done>=fctx->ts_server_connect_start);
				const u64_t server_connect_time = fctx->ts_server_connect_done - fctx->ts_server_connect_start;
	...
	inline static void dns_resolve_error(int const& code, WWRP<wawo::ref_base > const& cookie_) {
		...
		if (code == E_DNS_TEMPORARY_ERROR && (fctx->dns_try_time) < 5) {
			WWRP<wawo::timer> retry_timer = wawo::make_ref<wawo::timer>( std::chrono::milliseconds((fctx->dns_try_time)<<1),
				[fctx, code] (WWRP<wawo::timer> const& t) {
	...
	public:
		...
		_read_begin:
			...
			case CONNECT:
			{
				...
				if (fctx->client_up_first_packet->len()<4) {
					return;
				}

				const u8_t cmd = fctx->client_up_first_packet->read<u8_t>();
				if (cmd != C_CONNECT) {
				...
				if (fctx->client_up_first_packet->len()) { goto _read_begin; }
			}
			break;
			case READ_DST_ADDR:
			{
			...
			case SERVER_CONNECTED:
			{
				WAWO_ASSERT(fctx->client_up_first_packet->len() == 0);
	...
	void stream_accepted(WWRP<wawo::net::channel> const& ch)
	{
		...
		WWRP<wawo::net::channel_handler_abstract> h = wawo::make_ref<server_stream_handler>();
		ch->pipeline()->add_last(h);
	}
}
#endif